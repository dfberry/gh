...
export async function runCollaborators(client: GitHubClient, args: Args) {
  ...
  for (const repoFull of repos) {
    ...
    const r = await wrapGitHubRest(() => security.listCollaborators(client, owner, repo));
    if (r.ok) {
      results.push({ owner, repo, collaborators: r.data, status: 'ok' });
    } else {
      ...
      results.push({ owner, repo, collaborators: null, message, status: r.status || 'error' });
    }
  }
  return results;
}
