...
describe('cli repo input test', () => {
  it('evaluate-actions is invoked once per repo via runner', async () => {
    const tmp = './tmp-evaluate-actions-input.json';
    ...
    const mockImport = vi.fn(async (_: string) => ({ evaluateActionsCommand: spy }));

    const runner = makeRunner('../commands/evaluate-actions.js', 'evaluateActionsCommand', mockImport);
