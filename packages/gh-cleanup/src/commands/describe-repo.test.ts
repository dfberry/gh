...
describe('cli repo input test', () => {
  it('describe-repo is invoked once per repo via runner', async () => {
    const tmp = './tmp-describe-repo-input.json';
    ...
    const mockImport = vi.fn(async (_: string) => ({ describeRepoCommand: spy }));

    const runner = makeRunner('../commands/describe-repo.js', 'describeRepoCommand', mockImport);
