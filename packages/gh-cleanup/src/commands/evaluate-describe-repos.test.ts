...
describe('cli repo input test', () => {
  it('describe-repos is invoked once per repo via runner', async () => {
    const tmp = './tmp-describe-repos-input.json';
    ...
    const mockImport = vi.fn(async (_: string) => ({ describeReposCommand: spy }));

    const runner = makeRunner('../commands/evaluate-describe-repos.js', 'describeReposCommand', mockImport);
