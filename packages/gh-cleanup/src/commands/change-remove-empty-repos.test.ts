...
describe('cli repo input test', () => {
  it('delete-empty-repos is invoked once per repo via runner', async () => {
    const tmp = './tmp-delete-empty-repos-input.json';
    ...
    const mockImport = vi.fn(async (_: string) => ({ deleteEmptyReposCommand: spy }));

    const runner = makeRunner('../commands/change-remove-empty-repos.js', 'deleteEmptyReposCommand', mockImport);
