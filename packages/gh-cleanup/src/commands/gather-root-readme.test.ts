...
describe('cli repo input test', () => {
  it('gather-root-readme is invoked once per repo via runner', async () => {
    const tmp = './tmp-gather-root-readme-input.json';
    ...
    const mockImport = vi.fn(async (_: string) => ({ gatherRootReadmeCommand: spy }));

    const runner = makeRunner('../commands/gather-root-readme.js', 'gatherRootReadmeCommand', mockImport);
