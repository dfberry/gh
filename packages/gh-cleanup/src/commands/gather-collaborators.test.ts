...
describe('cli repo input test', () => {
  it('gather-collaborators is invoked once per repo via runner', async () => {
    const tmp = './tmp-gather-collaborators-input.json';
    ...
    const mockImport = vi.fn(async (_: string) => ({ collaboratorsCommand: spy }));

    const runner = makeRunner('../commands/gather-collaborators.js', 'collaboratorsCommand', mockImport);
